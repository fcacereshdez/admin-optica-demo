...
{
    public partial class SalesByConsultantParams : Form
    {
        public SalesByConsultantParams()
        {
            InitializeComponent();
        }
        ...
        private void btn_generate_report_Click(object sender, EventArgs e)
        {
            ...
            if (rb_all.Checked == true)
            {
                ...
                ReportDataSource rds = new ReportDataSource("ds_sales_by_consultant", commonController.SelectAllSalesByConsultantAll());
                viewerForm.rv_viewer.Reset();
                viewerForm.rv_viewer.ProcessingMode = ProcessingMode.Local;
                viewerForm.rv_viewer.LocalReport.ReportEmbeddedResource = "Presentation.View.Reports.SalesByConsultant.rdlc";
                viewerForm.rv_viewer.LocalReport.DataSources.Add(rds);
                messageOutLoader.Close();
                InsertAction("generó un reporte de todas las ventas por asesor.");
                Close();
            ...
            else
            {
                ...
                Application.DoEvents();
                System.Threading.Thread.Sleep(4000);
                ReportDataSource rds = new ReportDataSource("ds_sales_by_consultant", commonController.SelectAllSalesByConsultant(dtp_from.Value.ToString("yyyy-MM-dd"), dtp_to.Value.ToString("yyyy-MM-dd")));
                viewerForm.rv_viewer.Reset();
                viewerForm.rv_viewer.ProcessingMode = ProcessingMode.Local;
                viewerForm.rv_viewer.LocalReport.ReportEmbeddedResource = "Presentation.View.Reports.SalesByConsultant.rdlc";
                viewerForm.rv_viewer.LocalReport.DataSources.Add(rds);
        ...
        private void SalesByConsultantParams_Load(object sender, EventArgs e)
        {
